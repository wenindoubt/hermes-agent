...
def test_parse_dict_entry_with_label():
    out = parse_reaction_feedback(
        {"thumbsup": {"label": "ok", "instruction": "Record positive feedback."}}
    )
    assert out == {"thumbsup": ReactionFeedbackEntry(instruction="Record positive feedback.", label="ok")}
...
def test_parse_skips_entry_missing_instruction(caplog):
    with caplog.at_level(logging.WARNING, logger="gateway.reaction_feedback"):
        out = parse_reaction_feedback({"brain": {"label": "x"}})
    assert out == {}
...
def test_parse_skips_non_string_non_dict_value(caplog):
    with caplog.at_level(logging.WARNING, logger="gateway.reaction_feedback"):
        out = parse_reaction_feedback({"brain": 123})
    assert out == {}
...
def test_build_uses_label_when_present():
    entry = ReactionFeedbackEntry(instruction="Do the thing.", label="accepted")
    text = build_reaction_feedback_text(
        entry, emoji="thumbsup", message_ts="1.2", reacted_text="Hi"
    )
    assert "[Slack reaction feedback workflow]" in text
    assert "Reaction: :thumbsup: (accepted)" in text
